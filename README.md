...
This is a fork of Kromtec's Legends Viewer. Built and optimized for static frontend serving from server for online audience consumption and distribution of world data. Built for streamers and content creators.
...
**Features:** World overview with bookmarks and thumbnails, generated maps from XML exports, paginated browsing of sites/entities/figures/events, interactive Leaflet maps, family trees, efficient loading of large legends XML via XmlReader, and search that understands both the English and dwarven names (accent-insensitive).
...
## Dwarven language dictionary

-Todo: partial implementation, search and index references only, not on entity pages yet.

- The backend ships with `LegendsViewer.Backend/Data/dwarven-dictionary.json` and `english-root-map.json`, generated from the official DF Wiki raw files [`language_DWARF.txt`](https://github.com/DF-Wiki/DFRawFunctions/blob/master/raws/v50/language_DWARF.txt) and [`language_words.txt`](https://github.com/DF-Wiki/DFRawFunctions/blob/master/raws/v50/language_words.txt).
- To refresh the dictionaries when DF releases new raws, verify the DF Wiki has published the version you need (for example `raws/v50/...`), and update the source URLs in `Scripts/DwarvenDictionaryGenerator/Program.cs` if necessary.
- Run:
  ```bash
  dotnet run --project Scripts/DwarvenDictionaryGenerator
  ```
  The script fetches both raw files from the same directory and writes the dwarven dictionary and the English root map (for resolving inflected forms like "princesses" → "PRINCESS").
  Optional flags:
  - `--source <url-or-path>`: override the dwarven raw file location (local path or URL). The words file is derived from the same directory (e.g. `language_words.txt`).
  - `--words-source <url-or-path>`: override the words raw file location.
  - `--output <path>`: change the destination dictionary JSON file.
  - `--output-root-map <path>`: change the destination root map JSON file.
  - `--dry-run`: parse and print a preview without writing.
  - `--validate-only`: compare the current files to the latest raws without writing (fails if counts differ).
- The script regenerates both JSON files in-place; commit the updated files alongside your code changes.
- Servers can override the dictionary location via configuration key `DwarvenDictionary:Path` (appsettings/environment variable) if you store it elsewhere. The root map is expected in the same directory.
- Search bars throughout the UI now support English names, dwarven translations (e.g., `ngathsesh`), inflected forms (e.g. plurals like "princesses"), and ASCII-only spellings thanks to the generated aliases.
